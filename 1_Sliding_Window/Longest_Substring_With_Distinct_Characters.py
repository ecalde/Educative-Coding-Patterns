...
Solution
    ...
    remember the last index of each character we have processed. Whenever we get a duplicate character,
...
Space Complexity
    The algorithm’s space complexity will be O(K), where KK is the number of distinct characters in 
    the input string. This also means K<=N, because in the worst case, the whole string might not 
    ...
    we can say that the algorithm runs in fixed space O(1); in this case, we can use a fixed-size array 
